document.addEventListener('DOMContentLoaded', () => {
  ...
  function createNote() {
    ...
    // 添加动画类
    const newNoteEl = noteListEl.querySelector(`[data-id="${newNote.id}"]`);
    if (newNoteEl) {
      newNoteEl.classList.add('new-note');
      // 动画结束后移除类，以免影响后续交互（虽然这里可能不需要，因为每次render都会重置）
      setTimeout(() => newNoteEl.classList.remove('new-note'), 500);
    }
